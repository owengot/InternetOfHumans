...
export default {
  ...
  methods: {
    ...
    draw() {
      ...
      entering
        .merge(trail)
        .attr("transform", (d, i) => "translate(" + i * (this.itemWidth + this.spacing) + ", 0)")
        .transition(500)
        .style("opacity", d => (this.items.indexOf(d) >= originIndex ? 1 : 0.5))

      // Now move and update the percentage at the end.
      select(this.$el)
        .select(".trail")
        .select(".endlabel")
        .attr("x", this.items.length * (this.itemWidth + this.tailWidth / 2) + this.spacing * 2)
        .attr("y", this.itemHeight / 2)
        .attr("dy", "0.25em")
...
.default {
  ...
  width: 210px;
  background: RGBA(127, 201, 127, 1);
}
.bread-sequence >>> g:nth-child(3) {
